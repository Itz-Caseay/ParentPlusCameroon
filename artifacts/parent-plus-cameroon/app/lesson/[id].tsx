...
export default function LessonPlayerScreen() {
  ...
  const lesson = useMemo(
    () => lessons.find((item) => item.id === id) ?? lessons[0],
    [id],
  );
    ...
    >
      ...
      <View style={[styles.video, { backgroundColor: colors.foreground }]}>
        <View style={styles.videoTop}>
          <View style={styles.videoBadge}>
            ...
            />
            <Text style={styles.videoBadgeText}>OFFLINE READY</Text>
          </View>
          <Text style={styles.videoTime}>{Math.round(progress)}%</Text>
        </View>
        <View style={presenterStyles.presenter}>
          <Image
            source={require("../../assets/images/ai-presenter.png")}
            style={presenterStyles.presenterImage}
          />
          <Animated.View
            style={[
              presenterStyles.gestureHand,
              {
                backgroundColor: colors.accent,
                transform: [
                  {
                    translateY: hand.interpolate({
                      inputRange: [0, 1],
                      outputRange: [0, -16],
                    }),
                  },
          ...
          <Animated.View
            style={[
              presenterStyles.gestureHand,
              presenterStyles.gestureHandRight,
              {
                backgroundColor: colors.accent,
                transform: [
                  {
        ...
        <Text style={styles.signCaption}>{lesson.title[language]}</Text>
        <Text style={styles.signSubcaption}>
          {playing
            ? "AI presenter is signing with the voice"
            : "AI presenter · captions on"}
        </Text>
        <Pressable
          testID="play-lesson"
        ...
        >
          ...
          />
          <Text style={[styles.playText, { color: "#f4b39a" }]}>
            {playing ? "Pause lesson" : "Play with signing"}
          </Text>
        </Pressable>
      ...
      <View style={styles.detailSection}>
        <Text style={[styles.sectionTitle, { color: colors.foreground }]}>
          {language === "FR" ? "Détails de la leçon" : "Lesson details"}
        </Text>
        {lesson.details[language].map((point, index) => (
          <View key={`${lesson.id}-detail-${index}`} style={styles.detailRow}>
            <View
        ...
        >
          ...
          {currentQuestion.options.map((option, index) => (
            <Pressable
              ...
              style={[
                ...
                {
                  borderColor:
                    choice === index ? colors.primary : colors.border,
                  backgroundColor:
                    choice === index ? colors.secondary : colors.background,
                },
              ]}
            >
          ...
          {score === null ? (
            ...
            <View
              style={[
                styles.result,
                { backgroundColor: score ? colors.secondary : colors.accent },
              ]}
            >
              <Feather
                name={score ? "award" : "refresh-cw"}
                size={18}
...
const styles = StyleSheet.create({
  ...
  video: { borderRadius: 24, padding: 16, minHeight: 360 },
  videoTop: {
    flexDirection: "row",
    justifyContent: "space-between",
    alignItems: "center",
  },
  videoBadge: { flexDirection: "row", alignItems: "center", gap: 5 },
  videoBadgeText: {
    color: "#fffdf9",
  ...
  eyeRow: { flexDirection: "row", gap: 22, marginTop: 4 },
  eye: { width: 6, height: 8, borderRadius: 4, backgroundColor: "#17352d" },
  smile: {
    width: 20,
    height: 9,
    borderBottomWidth: 2,
  ...
  playText: { fontFamily: "Inter_600SemiBold", fontSize: 13 },
  progressTrack: {
    height: 4,
    borderRadius: 2,
    backgroundColor: "#496259",
    marginTop: 14,
    overflow: "hidden",
  },
  ...
  title: { fontFamily: "Inter_700Bold", fontSize: 23, lineHeight: 28 },
  ...
  audioText: { fontFamily: "Inter_600SemiBold", fontSize: 10 },
  body: {
    fontFamily: "Inter_400Regular",
    fontSize: 14,
    lineHeight: 22,
  ...
  quiz: { borderWidth: 1, borderRadius: 18, padding: 16, marginTop: 24 },
  quizHeading: { flexDirection: "row", gap: 8, alignItems: "center" },
  quizTitle: { fontFamily: "Inter_700Bold", fontSize: 16 },
  question: {
    fontFamily: "Inter_600SemiBold",
    fontSize: 15,
    lineHeight: 21,
  ...
  tryAgain: { fontFamily: "Inter_700Bold", fontSize: 12, marginLeft: 26 },
});
